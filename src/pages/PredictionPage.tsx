...
export function PredictionPage() {
    ...
    const makePrediction = async (data: WineFormData) => {
        ...
        try {
            ...
            const errorMessage = error?.response?.data?.message || error?.message || 'Failed to get prediction'
                        ...
                        <CardContent className="p-8">
                            {isLoading ? (
                                <div className="text-center py-16">
                                    ...
                                    />
                                    <p className="text-cream-200 text-lg">Analyzing wine properties...</p>
                                ...
                                >
                                    {/* Quality Score */}
                                    <div className={`bg-linear-to-r ${qualityInfo.gradient} p-0.5 rounded-lg`}>
                                        <div className="bg-wine-950 rounded-lg p-8 text-center">
                                            ...
                                            <p className="text-cream-200 text-xl mb-4">Predicted Quality Score</p>
                                            <div className="flex items-center justify-center gap-3">
                                                <span className={`px-4 py-2 rounded-full text-sm font-semibold text-white bg-linear-to-r ${qualityInfo.gradient}`}>
                                                    {qualityInfo.category}
                                                </span>
                                            </div>
                                            {result.confidence && (
                                                <div className="mt-6">
                                                    <p className="text-cream-400/80 text-sm mb-2">Confidence Level</p>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                        {formData && Object.entries(formData).map(([key, value]) => (
                                            <div key={key} className="bg-wine-800/30 rounded-lg p-4">
                                                <p className="text-cream-400/60 text-xs mb-1 capitalize">
                                                    {key.replace(/_/g, ' ')}
                                                </p>
                                                <p className="text-cream-200 font-semibold">{value}</p>
                                            </div>
                                        ))}
                                    </div>
                                </motion.div>
                                ...
                                <div className="text-center py-16">
                                    ...
                                    <p className="text-cream-200 text-lg mb-2">Failed to get prediction</p>
